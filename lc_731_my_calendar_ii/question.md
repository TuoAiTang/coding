...
「难度」: Medium
「标签」: 设计、线段树、二分查找、有序集合
「题目」: 实现一个 MyCalendar 类来存放你的日程安排。如果要添加的时间内不会导致三重预订时，则可以存储这个新的日程安排。
